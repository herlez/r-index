#!/bin/bash -l
#SBATCH -N 38 --constraint=cstd01 --ntasks-per-node=20 --mem=60G -t 02:00:00 --output=/work/smarherl/rindex_results/results_%j.txt -p short
...
do
mpirun --mca osc ^rdma -n 760  ./bench 1 $F ${F}.q30 -q 1000000
...
for F in /work/smarherl/matext/dna.128GiB/dna.128GiB
do
mpirun --mca osc ^rdma -n 480 ./bench 1 $F ${F}.q30 -q 1000000
done

#37.5 * 20 = 750
for F in /work/smarherl/matext/dna/dna
do
mpirun --mca osc ^rdma -n 760 ./bench 1 $F ${F}.q30 -q 1000000
done
